...
func TestResolverRejectsPrivateSymlinkTargets(t *testing.T) {
	root := t.TempDir()
	if err := os.Mkdir(filepath.Join(root, ".private"), 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".private", "secret.txt"), []byte("secret"), 0644); err != nil {
		t.Fatal(err)
	}
	for alias, target := range map[string]string{
		"public.txt": ".private/secret.txt",
		"public":     ".private",
	} {
	...
	for _, path := range []string{"/", "/public.txt", "/public/secret.txt"} {
		_, _, err := resolver.ResolvePath(path)
		if path == "/" && err != nil {
			t.Errorf("root resolution: %v", err)
		} else if path != "/" && !errors.Is(err, ErrPrivatePath) {
			t.Errorf("ResolvePath(%q) = %v, want private path", path, err)
		}
	}
}
